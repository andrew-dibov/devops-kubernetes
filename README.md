...
Создание облачной инфраструктуры с последующим развертыванием и конфигурацией отказоустойчивого Kubernetes-кластера [проекта](https://github.com/andrew-dibov/devops). Создание виртуальных машин, группы безопасности и балансировщиков нагрузки под API и Ingress. Автоматическая установка и конфигурация ключевых компонентов Kubernetes, инициализация control plane, подключение worker-узлов и развертывание Ingress NGINX, Prometheus Stack и Atlantis.
...
### Слой 2 : Облачная инфраструктура : Terraform
...
| :-- | :-- | :-- | :-- | :-- |
| **Ingress** | `TCP` | `22` | `10.0.1.0/24`, `10.0.2.0/24` | Администрирование и конфигурация через бастион |
...
| **Ingress** | `ANY` | `ANY` | `10.1.1.0/24`, `10.1.2.0/24` | Коммуникация кластера |
| **Egress** | `ANY` | `ANY` | `0.0.0.0/0` | Доступ в интернет через NAT |
| **Egress** | `ANY` | `ANY` | `10.1.1.0/24`, `10.1.2.0/24` | Коммуникация кластера |
...
Bash-скрипт запрашивает авторизационные данные и выполняет серию Ansible-плейбуков :
...
| **GitOps & CI/CD** | Atlantis + GitHub App | Реализация автоматического планирования/применения PR через вебхуки GitHub App |
